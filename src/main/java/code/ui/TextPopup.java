...
public class TextPopup implements TextReceiver
{
    ...
    public Runnable onConfirming;
    public Runnable onCanceling;

    public TextPopup(String title, boolean digitonly, Consumer<String> receiver, Runnable onConfirming, Runnable onCanceling) {
    ...
    public void confirm() {
        if(digitonly) textField = textField.trim();
        if (textField.equals("")) {
            return;
        }
    ...
    public boolean acceptCharacter(char c) {
        return !this.digitonly || Character.isDigit(c) || c == '-';
    }
